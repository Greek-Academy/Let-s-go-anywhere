...
入力ツールは51件（PCのChromium・スマホ幅のChromium・スマホ幅のWebKit）を確認します。形式・日時境界・根拠参照・写真条件、JSON往復、元の入力の保持、キーボード操作、通常の保存領域を読み書きしないこと、外部への自動通信がないこと、HTML文字列の表示、配信物への分離に加え、実際の一覧・詳細・保存・5タブ・削除を検証します。通常アプリ側は既存の252件も実行します。配信復旧の3件は未コミット変更のないビルドが必要です。
