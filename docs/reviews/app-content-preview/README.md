...
- 入力ツール51件：PCとスマホ幅のChromium、スマホ幅のWebKitで検証。名称変更から一覧・詳細・保存・戻る、5タブ、設定からの削除、写真の利用条件、入力エラーを確認。確認画面の読込失敗から再試行しても元の入力が残ることを確認。
- 普段の保存領域を読む・書く・削除する処理を監視し、確認画面ではいずれも呼ばれないことを確認。非公開のメモ・許諾記録・情報源URLは表示用データに含めない。
- 通常アプリ249件は作業フォルダで成功。配信復旧の3件も未コミット変更のない別の検証フォルダで成功し、合計252件を確認。復旧処理の単体検査9件も成功。
