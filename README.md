...
開発状況を確認したい方は、まず[今できることと次に進めること](docs/PROJECT_STATUS.md)をご覧ください。モックと実運用の違い、次の候補、試していただきたい3つの操作をまとめています。
...
Issue #33の先行部分として、静的配信物を保存し、欠損・変更を検査して新しいフォルダへ復元するコマンドを追加しました。既存ファイル・普段のブラウザの保存データは上書きしません。[操作手順・対象・残作業](docs/RELEASE_RECOVERY.md)を参照してください。Node標準機能で動作し、追加サービス費用は0円です。初期費用・運用費を含む累計3,000円の上限で進めます。

## 掲載情報の入力ツール（開発用）

Issue #17の先行部分として、情報源・確認日・写真利用条件を入力してカードを確認するローカルツールを追加しました。

```bash
npm run review  # http://127.0.0.1:4180/
```

「確認用サンプル」から試せます。入力はタブ内のメモリで扱い、必要なときだけJSONとして保存します。[操作手順・確認画像](docs/reviews/content-review/README.md) と [入力形式・検査・残作業](docs/CONTENT_REVIEW.md) を参照してください。施設確認・写真の許諾・公開承認を行う機能は後続です。
...
| 行きたい | お出かけと車候補の切り替え、保存・解除の同期、SNS URL追加・削除、内容未確認の表示、遷移先確認後に元のページを開く |
...
- 架空の施設・車拠点・講習会社は案内モーダルのみで、外部サイトや実地図を開きません。本人が保存したHTTPS URLは、移動先を確認してから開けます。外部遷移は予約・相談完了として記録しません。
...
保存データが壊れている場合は、読み取れる部分で画面を開き、元のデータを自動で上書きしません。画面上部の保存状態から設定へ進み、元データのダウンロード、保存失敗後の再試行、確認付きの初期化ができます。[操作結果・確認画像](docs/reviews/storage-recovery/README.md) と [検証の仕様・限界](docs/STORAGE_RECOVERY.md) を参照してください。本番の認証・サーバー同期・DB復元機能とは別の仕組みです。
