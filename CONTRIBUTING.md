...
最新の全体整理は[今できることと次に進めること](docs/PROJECT_STATUS.md)とIssue #1を参照します。Issue数やテスト件数だけを進捗として扱いません。

- 開発前：できるようになること、今やる理由、今回の範囲、追加費用、完了の基準を平易に説明します。
- 開発後：変更の目的と影響、ユーザーが試す1〜3操作、期待する結果、開発側の検証、未確認事項を分けます。画面確認が不要なら理由を説明します。
- 保存データ・外部送信・費用への影響は、画面変更の有無にかかわらず記載します。正常系・失敗時・既存機能への影響を何で確認したか示し、件数は補足にします。
- ユーザーにコードの読解を求めず、期待する体験になっているかを確認してもらいます。ブラウザ相当の検証と実機、本物の情報とサンプル、実送信とデモを区別します。
- 既存の承認・保留指示を引き継ぎます。未決定事項が必要な場合は、具体的な判断材料を揃えて相談します。
- 一部実装のIssueには残作業を残します。受け入れ条件を満たしたIssueは、根拠と残る別Issueを記録して完了に整理します。
